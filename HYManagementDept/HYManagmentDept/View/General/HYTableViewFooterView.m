...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _loadText = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 12.0f, 320.0f, 20.0f)];
...
- (void)layoutSubviews
{
    ...
    CGSize tsize = [_loadText.text sizeWithFont:_loadText.font];
    CGFloat w = tsize.width + 20;
    CGFloat x = CGRectGetWidth(self.superview.frame)/2 - w/2;
    _loadView.frame = CGRectMake(x, CGRectGetMidX(self.superview.bounds)-10, 20, 20);
    _loadText.frame = CGRectMake(x+20, CGRectGetMinY(_loadView.frame), tsize.width, tsize.height);
}
