...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    self.baseViewController.pageController.navigationItem.leftBarButtonItem = nil;
    if (self.baseViewController.pageController)
    {
        self.baseViewController.pageController.navigationItem.rightBarButtonItem = [self cityItem];
    }
    else
    {
...
//    [HYLoadHubView show];
    ...
    [[HYLocationManager sharedManager] getCacheAddressInfo:^(BOOL success, NSDictionary *addrInfo)
    {
        [HYLoadHubView dismiss];
        if (success)
        {
            NSString *city = [addrInfo objectForKey:kHotelDefCity];
            b_self.cityName = city;
            [b_self.cityBtn setTitle:city forState:UIControlStateNormal];
            NSDictionary *location = [addrInfo objectForKey:kUserLocation];
            _coor.latitude = [[location objectForKey:@"lat"] doubleValue];
            _coor.longitude = [[location objectForKey:@"long"] doubleValue];
