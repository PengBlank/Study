...
- (void)layoutSubviews
{
    ...
    CGFloat w = CGRectGetWidth(self.frame) - 15;
    _contentView.frame = CGRectMake(0, 15, w, w);
    
    w -= 15;
    _deleteView.frame = CGRectMake(w, 0, 30, 30);
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view == _contentView)
    {
        ...
        action(self.touchTarget, NSSelectorFromString(_touchAction), self);
...
//                               withObject:self];
    ...
    {
        void (*action)(id, SEL, id) = (void (*)(id, SEL, id))objc_msgSend;
        action(self.deleteTarget, NSSelectorFromString(_deleteAction), self);
