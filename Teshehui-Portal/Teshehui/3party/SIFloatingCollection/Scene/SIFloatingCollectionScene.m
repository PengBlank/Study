...
- (void)update:(NSTimeInterval)currentTime {
    ...
    for (SKNode *node in self.floatingNodes) {
        ...
        node.physicsBody.linearDamping = distanceFromCenter > 100 ? 0.5 : (0.5 + (100-distanceFromCenter)/100);
    }
...
- (void)addChild:(SKNode *)node {
    if ([node isKindOfClass:[SIFloatingNode class]]) {
        SIFloatingNode *child = (SIFloatingNode *)node;        [self configureNode:child];
