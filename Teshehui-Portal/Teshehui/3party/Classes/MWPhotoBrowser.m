...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        ...
        [btn setImage:[UIImage imageNamed:@"share_icon_normal"] forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:@"share_icon_press"] forState:UIControlStateHighlighted];
        [btn addTarget:self
...
- (CGPoint)contentOffsetForPageAtIndex:(NSUInteger)index {
	CGFloat pageWidth = _pagingScrollView.bounds.size.height;
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            ...
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedString(@"图片", @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedString(@"图片", @"Used in the context: '3 photos'");
            }
