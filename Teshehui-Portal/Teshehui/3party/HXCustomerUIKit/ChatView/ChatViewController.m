...
- (void)setupBarButtonItem
{
    ...
    [backButton setImage:[UIImage imageNamed:@"btn_back_arrow_focus"] forState:UIControlStateNormal];
    [backButton addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithCustomView:backButton];
    [self.navigationItem setLeftBarButtonItem:backItem];
    
    UIButton *clearButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
    [clearButton setImage:[UIImage imageNamed:@"cart_drop"] forState:UIControlStateNormal];
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                if (model.isSender) {
                    HYUserInfo *user = [HYChatManager getCurUserInfo];
                    NSURL *URL = [NSURL URLWithString:user.userLogo.defaultURL];
                    [cell.headImageView sd_setImageWithURL:URL placeholderImage:[UIImage imageNamed:@"mine_default"] options:SDWebImageRefreshCached];
                }
                else
                {
...
- (NSDictionary*)getUserInfoAttribute
{
    ...
    if (user.realName)
    {
        [visitor setObject:user.realName
                    forKey:@"trueName"];
    }
    
    if (user.nickName) {
        [visitor setObject:user.nickName
                    forKey:@"userNickname"];
    }
