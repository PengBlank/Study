...
//                    label.font = [UIFont systemFontOfSize:(currentDeviceType() == iPhone6Plus) ? (XTSegmentControlItemFont + 1) : (currentDeviceType() == iPhone6 ? XTSegmentControlItemFont : XTSegmentControlItemFont - 2)];
//                    self.backgroundColor = [UIColor whiteColor];
                    label.font = [UIFont systemFontOfSize:12];
                    label.textAlignment = NSTextAlignmentCenter;
                    label.text = title;
                    label.textColor = [UIColor colorWithHexColor:@"222222" alpha:1];
...
- (void)setSelected:(BOOL)selected
{
    switch (_type) {
        ...
        {
            if (_titleLabel) {
                [_titleLabel setTextColor:(selected ? [UIColor colorWithHexColor:@"b80000" alpha:1]:[UIColor colorWithHexColor:@"222222" alpha:1])];
            }
            if (_titleIconView) {
...
@property (nonatomic , assign) BOOL isNoSeparateLine;

@property (nonatomic , strong) NSMutableArray *itemFrames;
@property (nonatomic , strong) NSMutableArray *itemFramesCopy;
...
- (instancetype)initWithFrame:(CGRect)frame Items:(NSArray *)titleItem withSeparateLine:(BOOL)isNoSeparateLine selectedBlock:(XTSegmentControlBlock)selectedHandle{
    
    self.isNoSeparateLine = isNoSeparateLine;
    if (self = [self initWithFrame:frame Items:titleItem withIcon:NO]) {
        self.block = selectedHandle;
    }
    return self;
}
...
- (void)initItemsWithTitleArray:(NSArray *)titleArray withIcon:(BOOL)isIcon
{
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        ...
        for (int i = 0; i < titleArray.count; i++) {
            ...
            XTSegmentControlItem *item = [[XTSegmentControlItem alloc] initWithFrame:rect title:title type: isIcon ? XTSegmentControlItemTypeTitleAndIcon : XTSegmentControlItemTypeTitle];
            if (!isIcon && i == 0) {
    ...
    if (!_isNoSeparateLine) {
        ...
        for (int i=1; i<_itemFrames.count; i++) {
            ...
            UIView *lineView  = [[UIView alloc] initWithFrame:CGRectMake(
                                                                         ...
                                                                         0.5,
                                                                         14)];
            lineView.backgroundColor = [UIColor colorWithHexColor:@"dddddd" alpha:1];
...
- (void)addRedLine:(NSInteger)index
{

    if (!_lineView) {
    
        CGRect tmpRect = [_itemFrames[0] CGRectValue];
       // CGRect rect;
        if (_isNoSeparateLine) {
//            _itemFramesCopy = _itemFrames;
            NSMutableArray *arr = [NSMutableArray array];
            
            for (int i = 0; i < _itemFrames.count; i++) {
                
                XTSegmentControlItem *tmpItem = _items[i];
                CGRect rect = [_itemFrames[i] CGRectValue];
                
                NSString *str = tmpItem.titleLabel.text;
                CGSize size = [str sizeWithFont:[UIFont systemFontOfSize:(currentDeviceType() == iPhone6Plus) ? (XTSegmentControlItemFont + 1) : (currentDeviceType() == iPhone6 ? XTSegmentControlItemFont : XTSegmentControlItemFont - 2)] constrainedToSize:CGSizeMake(kScreen_Width/_items.count, rect.size.height)];
                
                if (i == 0) {
                    tmpRect.origin.x = rect.size.width/2 - size.width/2;
                    tmpRect.size.width = size.width + 5;
                }

                
                rect.origin.x = rect.size.width/2 - size.width/2 + (rect.size.width * i) - 2.5;
        ...
        if (!_isIcon) {
            _lineView = [[UIView alloc] initWithFrame:CGRectMake(
                                                                 ...
                                                                 CGRectGetHeight(tmpRect) - XTSegmentControlLineHeight,
                                                                 CGRectGetWidth(tmpRect) - 2 * XTSegmentControlHspace,
                                                                 XTSegmentControlLineHeight)];
            _lineView.backgroundColor = [UIColor colorWithHexColor:@"b80000" alpha:1];
            [_contentView addSubview:_lineView];
        }
        
        UIView *bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(tmpRect)-0.5, CGRectGetWidth(self.bounds), 0.5)];
        bottomLineView.backgroundColor = [UIColor colorWithHexColor:@"dddddd" alpha:1];
        ...
        if (index != _currentIndex) {
            ...
            if (_currentIndex < 0) {
                ...
                [UIView animateWithDuration:0.3 animations:^{
                    ...
                    [weakSelf.items enumerateObjectsUsingBlock:^(XTSegmentControlItem *item, NSUInteger idx, BOOL *stop) {
                        [item setSelected:NO];
                    }];
                    [curItem setSelected:YES];
