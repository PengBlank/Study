...
- (BOOL)isNil{
    ...
    if (self == nil ||  self == NULL || [self isEqualToString:@"null"] ||[self isKindOfClass:[NSNull class]]) {
        return YES;
    }
...
+ (NSString *)turnToSecurityNum:(NSString *)num
{
    if (num.length > 2)
    {
        NSInteger length = num.length;
        NSInteger number = length / 2; // 需要隐藏的数的个数
        NSInteger temp = length - number;
        if (temp % 2)
        {
            // 基数：被隐藏数的个数--
            number--;
        ...
        NSInteger begin = (length - number) / 2;
        ...
        num = [num stringByReplacingCharactersInRange:NSMakeRange(begin, number) withString:hideStr];
        return num;
    }
    else
    {
