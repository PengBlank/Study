...
@implementation SceneRefundRequest
...
-(NSMutableDictionary *)getJsonDictionary
{
    ...
    if (newDic && (NSNull *)newDic != [NSNull null])
    {
        ...
        NSDictionary *tmpDic = @{@"o2oOrderNo":self.o2oOrderNo,
                                 @"packageName":self.packageName,
                                 @"reason":self.reason
                                 };
        
        NSString *token = CenterToken; // 退款 用中心token
        NSString *action = @"";
        NSString *app_key = @"SceneRefund";
