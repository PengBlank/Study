...
{ // type 0为普通商家 1为桌球
    ...
    self.rpInfo = rpInfo;
    //彩色背景图
    NSArray *colorImageNames1 = @[@"coupons",@"yellow-coupons",@"red-coupons",@"green-coupons"];
    NSArray *colorImageNames_Billiards = @[@"coupons_Billiards",@"yellow-coupons_Billiards",@"red-coupons_Billiards",@"green-coupons_Billiards"];
    if (index > 3) {
        index = index%4;
    }
    switch (type) {
        case 0:
        {// 普通商家
            UIImage *colorIamge = [UIImage imageNamed:colorImageNames1[index]];
        ...
        case 1:
        {// 桌球 不需要判断现金券
            UIImage *colorIamge = [UIImage imageNamed:colorImageNames_Billiards[index]];
            [self.colorImageView setImage:colorIamge];
//            if (havePoint)
//            {// 现金券充足
//                [self.bgImageView setImage:nil];
...
//            
            ...
            [self.subLable setText:[NSString stringWithFormat:@"%@元",rpInfo.givenAmount]];
            WS(weakSelf);
            [self.subLable mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.centerY.mas_equalTo(weakSelf.colorImageView.mas_centerY);
                make.centerX.mas_equalTo(weakSelf.colorImageView.mas_centerX);
            }];
            // 扣除
            [self.deductLabel setText:@""];
