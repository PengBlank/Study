...
- (void)viewDidLoad {
   ...
   // [self cityItem]; //创建导航按钮item
    ...
    [_mySearchBar setFrame:CGRectMake(75,22, kScreen_Width - 120 , 40)];
...
//    if (!self.isNoScenePay) {
//        self.navigationItem.rightBarButtonItem = [self rightBarItem];
//        self.navigationItem.leftBarButtonItem = [self leftBarItem];
//    }else{
//        [self.navigationController.view addSubview:[self leftBtn]];
//    }
}
...
- (UIButton *)leftBtn{
    ...
    if (!_cityBtn) {
        ...
        cityButton.frame = CGRectMake(0, 27, 80, 30);
        [cityButton setImage:[UIImage imageNamed:@"icon_moregray"] forState:UIControlStateNormal];
        [cityButton setTitle:self.cityName ?:@"城市" forState:UIControlStateNormal];
        [cityButton setTitleColor:[UIColor colorWithHexString:@"0xb80000"] forState:UIControlStateNormal];
        [[cityButton titleLabel] setFont:[UIFont systemFontOfSize:12]];
        [cityButton addTarget:self action:@selector(cityButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [cityButton layoutButtonWithEdgeInsetsStyle:TYButtonEdgeInsetsStyleRight imageTitleSpace:0.0f];
...
- (UIBarButtonItem *)leftBarItem{
    ...
    {
        UIButton *cityButton = [UIButton buttonWithType:UIButtonTypeCustom];
        cityButton.frame = CGRectMake(0, 0, 80, 30);
        [cityButton setImage:[UIImage imageNamed:@"icon_moregray"] forState:UIControlStateNormal];
        [cityButton setTitle:self.cityName ?:@"城市" forState:UIControlStateNormal];
        [cityButton setTitleColor:[UIColor colorWithHexString:@"0xb80000"] forState:UIControlStateNormal];
        [[cityButton titleLabel] setFont:[UIFont systemFontOfSize:12]];
...
- (UIBarButtonItem *)rightBarItem{
    if (!_rightBarItem)
    {
        ...
        [switchButton setTitleColor:[UIColor colorWithHexString:@"0xb80000"] forState:UIControlStateNormal];
        [[switchButton titleLabel] setFont:[UIFont systemFontOfSize:12]];
...
#pragma mark -- 获取定位信息 --》loadData --》获取生活分类信息 --》获取区域分类信息 --》获取区域分类信息 --》初始化分类搜索视图 --》 商家列表数据请求
   ...
   [[HYLocationManager sharedManager] getCacheAddressInfo:^(HYLocateState state, HYLocateResultInfo *result)
     {
         ...
         else
         {
             ...
             [HYLoadHubView dismiss];
             weakSelf.myQRCode.hidden = YES;
            [weakSelf.baseTableView setShowsInfiniteScrolling:NO];

             [weakSelf.baseTableView configBlankPage:EaseBlankPageTypeNolocation hasData:NO hasError:HasError
                                   reloadButtonBlock:^(id sender) {
...
- (void)loadDate{
    ...
    dispatch_group_t group = dispatch_group_create(); //创建GCD group 是为了等数据请求全部执行完之后进行后续操作
...
- (void)setWithCity:(NSString *)city
{
    if (city)
    {
        ...
        [self.cityBtn layoutButtonWithEdgeInsetsStyle:TYButtonEdgeInsetsStyleRight imageTitleSpace:0];
        _isCityFresh = NO;
        _isSearch = NO;
        [HYLoadHubView show];
        [self loadDate];
