...
- (void)setupControllerSubviews
{
    ...
    _dynamicDiscountViewConstraint.constant = -(kDefaltConstraint+_noDiscountTextF.bounds.size.height); // 初始约束
...
- (IBAction)prompNoDiscountBtnDidClick:(id)sender {
    ...
    if (_prompNoDiscountButton.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [weakSelf figureConsumeMoney:weakSelf.oldIndex];
...
- (void)figureConsumeMoney:(NSInteger)index{
    ...
    _disCountValue = _disCount[index];
    _oldIndex = index;
    if ([money floatValue] >= [noDiscount floatValue]) {
        
        self.cashTextF.text = [NSString stringWithFormat:@"%.2f", (([money floatValue] - [noDiscount floatValue]) *[_disCount[index] floatValue] * 0.1 + [noDiscount floatValue])];

        
        NSString *pay = [NSString stringWithFormat:@"%.2f", ([money floatValue] - [noDiscount floatValue]) * (1 - [_disCount[index] floatValue] * 0.1)];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField == self.expenditureTextF) {
        ...
        if ([_inputMoney floatValue] >= [noDiscount floatValue]) {
            ...
            self.cashTextF.text = [NSString stringWithFormat:@"%.2f", (([_inputMoney floatValue] - [noDiscount floatValue]) *[_disCountValue floatValue] * 0.1 + [noDiscount floatValue])];
...
- (IBAction)goPayButtonDidclick { // 去支付事件处理
   ...
   // {"data":{"o2o_trade_no":"TQJGZ1457490585","c2b_trade_no":"112016DLM291645143","c2b_order_id":"10002"},"code":"0","msg":"主动付款创建订单"}
