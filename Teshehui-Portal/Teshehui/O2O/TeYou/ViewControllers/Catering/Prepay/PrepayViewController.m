...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell refreshUIDataWithModel:rpInfo Index:indexPath.row HavePoint:havePoint]; // 刷新数据
...
- (void)loadPrepayViewData
{
    ...
    if (kNetworkNotReachability) {
        ...
        DebugNSLog(@"网络异常");
        [self.myCollectionView configBlankPage:EaseBlankPageTypeNoNetwork hasData:NO hasError:[[NSError alloc] init] reloadButtonBlock:^(id sender) {
            [weakSelf loadPrepayViewData];
        }];
        return;
    }
    [HYLoadHubView show]; //??
...
//            [METoast toastWithMessage:@"无法连接服务器"];
            [weakSelf createPromptView:@"无法连接服务器"];
        }
...
- (void)prepayButtonClick
{
    ...
    [self.prepayButton setEnabled:NO]; // 按钮不可点击
    WS(weakSelf);
    HYUserInfo *userInfo = [HYUserInfo getUserInfo]; // 用户信息
    ...
    self.submitPRequest.coupon        = self.coupon;                    // 现金券
    ...
    [self.submitPRequest sendReuqest:^(id result, NSError *error) {
        ...
        [HYLoadHubView dismiss]; // load
        [self.prepayButton setEnabled:YES]; // 按钮可点击
        }];
...
- (void)gotoPay
{
    ...
    payVC.point = [self.coupon integerValue];     //  现金券
