...
//    NSString *url = [NSString stringWithFormat:@"%@?imageView2/1/w/200/h/200",@"http://ppt360.com/background/UploadFiles_6733/201012/2010122016291897.jpg"];
    ...
    // 钱
    [self.moneyLabel setText:[NSString stringWithFormat:@"¥%@",sbInfo.balance]];
    // 判断是否充值
    WS(weakSelf);
...
{
    [self.prepayButton setTag:index];
    [self.prepayButton addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
}
// 账单按钮
- (void)addBillButtonTarget:(id)target Action:(SEL)action Index:(NSInteger)index
{
    [self.billButton setTag:index];
    [self.billButton addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
}
