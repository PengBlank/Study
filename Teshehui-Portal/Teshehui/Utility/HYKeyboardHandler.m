...
- (void)keyboardWillChange:(NSNotification *)notification
{
    ...
    if (self.inputView)
    {
        CGRect frameInTable = [self.view convertRect:_inputView.frame
                                                   fromView:_inputView.superview];
        ...
        {
            [self.delegate inputView:self.inputView willCoveredWithOffset:offset];
        }
    }
}
...
{
    static HYKeyboardHandler *__sharedHandler = nil;
    if (!__sharedHandler) {
        __sharedHandler = [[HYKeyboardHandler alloc] init];
    }
    [__sharedHandler startListen];
