...
- (void)updateInfoResult:(HYGetUpdateInfoResponse *)response force:(BOOL)force
{
    NSString *appVersion =[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
    NSString *newversion = response.version_name;
    BOOL newVersion = [newversion caseInsensitiveCompare:appVersion] == NSOrderedDescending;
    if (newVersion)
    {
        _forceUpdate = response.force_update;
        
#ifdef ADHOC
        _trackViewUrl =  [NSString stringWithFormat:@"itms-services://?action=download-manifest&url=%@",
                          response.url];
...
#else
        [self checkNewVersionNeedAlert:NO];
#endif
    }
}
...
- (void)checkForceupdate
{
    ...
    [_request sendReuqest:^(id result, NSError *error) {
        if (!error && [result isKindOfClass:[HYGetUpdateInfoResponse class]])
        {
            ...
            [bself updateInfoResult:response
...
- (void)checkNewVersionNeedAlert:(BOOL)alert
{
    ...
    NSString *post=nil;
    post=[[NSString alloc]initWithFormat:@"id=%@",AppId];
