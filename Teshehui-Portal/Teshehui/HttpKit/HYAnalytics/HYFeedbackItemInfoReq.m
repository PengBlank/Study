...
@implementation HYFeedbackItemInfo

@end

@implementation HYFeedbackItemInfoReq
...
- (id)init
{
    ...
    if (self)
    {
        self.interfaceURL = @"http://xres.teshehui.com/xclk/fb";
    }
    return self;
}
...
- (NSMutableDictionary *)getJsonDictionary
{
    ...
    if (self.fb_type) {
        [newDict setObject:self.fb_type forKey:@"fb_type"];
    }
    ...
    if ([self.fb_detail count])
    {
        NSArray *dicArr = [HYFeedbackItemInfo arrayOfDictionariesFromModels:self.fb_detail];
//        NSString *json = [dicArr JSONString];
        if ([dicArr count])
        {
            [newDict setObject:dicArr
                        forKey:@"fb_detail"];
        }
    }
